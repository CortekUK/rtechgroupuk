...
const rentalSchema = z.object({
  ...
  advance_payment: z.coerce.number().min(0, "Advance payment cannot be negative").optional(),
}).refine((data) => {
...
const CreateRental = () => {
  ...
  const onSubmit = async (data: RentalFormData) => {
    ...
    try {
      ...
      // Generate all monthly charges for the rental period upfront
      // Must happen before payments so FIFO allocation has charges to allocate against
      await supabase.rpc("generate_rental_charges", { r_id: rental.id });
      ...
      // If advance payment, create a lump payment and apply it
      if (data.advance_payment && data.advance_payment > 0) {
        const advanceAmount = data.advance_payment;

        const { data: advancePayment, error: advancePaymentError } = await supabase
          .from("payments")
          .insert({
            customer_id: data.customer_id,
            rental_id: rental.id,
            ...
            payment_date: data.start_date.toISOString().split('T')[0],
        ...
        if (advancePaymentError) throw advancePaymentError;

        const { data: advanceResult, error: advanceProcessError } = await supabase.functions.invoke('apply-payment', {
          body: { paymentId: advancePayment.id }
        });

        if (advanceProcessError) {
          console.error('Advance payment processing error:', advanceProcessError);
          throw new Error(`Failed to process advance payment: ${advanceProcessError.message}`);
        }

        if (advanceResult && !advanceResult.ok) {
          console.error('Advance payment processing failed:', advanceResult);
          throw new Error(`Advance payment processing failed: ${advanceResult.error || 'Unknown error'}`);
        }
      }
      ...
      try {
        ...
        // Add advance payment as line item if present
        if (data.advance_payment && data.advance_payment > 0) {
          const approxMonths = data.monthly_amount > 0
            ? `~${(data.advance_payment / data.monthly_amount).toFixed(1)} months`
            : '';
          lineItems.push({
            description: `Advance Payment${approxMonths ? ` (${approxMonths})` : ''}`,
            quantity: 1,
            unit_price: data.advance_payment,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <CurrencyInput
                            ...
                            min={0}
                            step={1}
                            error={!!form.formState.errors.advance_payment}
                          />
                        </FormControl>
                        <FormMessage />
                        <FormDescription>
                          Amount paid upfront. Will be auto-allocated to outstanding charges via FIFO.
                          {field.value && field.value > 0 && watchedValues.monthly_amount ? (
                            <span className="block font-medium mt-1">
                              Covers ~{(field.value / watchedValues.monthly_amount).toFixed(1)} months at £{watchedValues.monthly_amount}/month
                            </span>
                          ) : null}
                        </FormDescription>
                      </FormItem>
        ...
        <div className="xl:col-span-1">
          <ContractSummary
            ...
            advancePayment={watchedValues.advance_payment}
          />
        </div>
      </div>
